...
public class JSHint {
	...
	public void configure(JsonObject configuration) {
		...
		try {
			...
			if (globalsValue != null) {
				...
				globals = context.evaluateString(scope, globalsExpr, "[globals]", 1,
						null);
			}
			configuration.remove("globals");
			String optionsExpr = "options = " + configuration.toString() + ";";
			options = context.evaluateString(scope, optionsExpr, "[options]", 1,
					null);
	...
	public boolean check(String code, List<ProblemHandler> handlerList) {
		...
		return check(new Text(code), handlerList);
	}
	...
	public boolean check(Text text, List<ProblemHandler> handlerList) {
		...
		if (code.trim().length() != 0) {
			...
			try {
				result = checkCode(context, code);
				if (!result && handlerList != null) {
					handleProblems(handlerList, text);
				}
			} finally {
				Context.exit();
			}
	...
	private void load(Reader reader) throws IOException {
		...
		try {
			...
			throw new IllegalArgumentException("Could not evaluate JavaScript input",
					exception);
		} finally {
			Context.exit();
		}
	...
	private void handleProblems(List<ProblemHandler> handlerList, Text text) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (error != null) {
				Problem problem = createProblem(error, text);
				for (ProblemHandler handler : handlerList) {
					handler.handleProblem(problem);
				}
			}
		}
	...
	ProblemImpl createProblem(ScriptableObject error, Text text) {
		...
		String message = reason.endsWith(".")
